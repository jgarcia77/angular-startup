...
var VividSeats = (function() {
    //private collection and accessors
      var _getRandomDate = function(offset) {
        var today = new Date();
        var date = new Date(today.getFullYear(), today.getMonth(), today.getDate(), 18, 0, 0);
        date.setDate(date.getDate() + offset );

        return date.toISOString();  
      }

      var _getRandomFutureDate = function() {
        var randomNumber = Math.ceil(Math.random() * 100);
        return _getRandomDate(randomNumber)

      }

      var _getRandomPastDate = function() {
